...
function showDashboard() {
  ...
  // Helper to detect super admin (by position or UID)
  function isCurrentSuperAdmin() {
    const pos = (currentUser && currentUser.position ? currentUser.position : '').toString().trim().toLowerCase();
    const uid = (currentUser && currentUser.uid ? currentUser.uid : '').toString().trim().toUpperCase();
    return pos === 'super admin' || uid === 'FFDFA0DA';
  }

  // Show/hide web attendance buttons for users with web access (including admins),
  // but NEVER show personal buttons to Super Admins
  const btnWebSignIn = document.getElementById('btnWebSignIn');
  const btnWebSignOut = document.getElementById('btnWebSignOut');
  
  if (currentUser.hasWebAccess && !isCurrentSuperAdmin()) {
...
function displayAttendance() {
  ...
  // Show "Sign Out All" button only if there are checked-in members and user is Super Admin
  if (btnSignOutAll) {
    const isSuperAdmin = !!(currentUser && (((currentUser.position || '').toLowerCase() === 'super admin') || ((currentUser.uid || '').toUpperCase() === 'FFDFA0DA')));
    if (isSuperAdmin && checkedInMembers.length > 0) {
...
function updateWebAttendanceStatus() {
  ...
  // Never show personal buttons for super admins
  const isSuper = ((currentUser.position || '').toLowerCase() === 'super admin') || ((currentUser.uid || '').toUpperCase() === 'FFDFA0DA');
  if (isSuper) {
...
async function performWebSignIn() {
  if (!currentUser) return;
  // Super admins don't use personal sign in
  const isSuper = ((currentUser.position || '').toLowerCase() === 'super admin') || ((currentUser.uid || '').toUpperCase() === 'FFDFA0DA');
  if (isSuper) return;
  if (!currentUser.hasWebAccess) {
...
async function performWebSignOut() {
  if (!currentUser) return;
  // Super admins don't use personal sign out
  const isSuper = ((currentUser.position || '').toLowerCase() === 'super admin') || ((currentUser.uid || '').toUpperCase() === 'FFDFA0DA');
  if (isSuper) return;
  if (!currentUser.hasWebAccess) {
    showNotification('Web access not granted for this account.', 'error');
