...
function showDashboard() {
  ...
  // Helper to detect if user is in Admin sheet (not by position in Member sheet)
  function isInAdminSheet() {
    if (!currentUser) return false;
    const uid = currentUser.uid.toString().trim().toUpperCase();
    // Check if user is in the admin list (from allMembers cache)
    const adminEntry = (allMembers || []).find(m => 
      m.uid === uid && m.isAdmin === true
    );
    return !!adminEntry;
  }

  // Show/hide web attendance buttons for users with web access
  // ONLY hide personal buttons for users in the Admin sheet
  const btnWebSignIn = document.getElementById('btnWebSignIn');
  const btnWebSignOut = document.getElementById('btnWebSignOut');
  
  if (currentUser.hasWebAccess && !isInAdminSheet()) {
