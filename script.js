      ...
      try {
        ...
        if (person) {
          currentUser.name = person.name || currentUser.name;
            ...
            const isAdminByPosition = (_pos === 'admin' || _pos === 'head admin');
...
async function handleLogin(e) {
  ...
  try {
    ...
    // Determine website admin access: either in Admins sheet OR position is Admin/Head Admin
    const _pos = (person.position || '').toString().trim().toLowerCase();
    const isAdminByPosition = (_pos === 'admin' || _pos === 'head admin');
    ...
    // Save session
    localStorage.setItem('attendanceSession', JSON.stringify(currentUser));
    // Reload the whole page so all data is freshly loaded for the new session
    // (prevents stale UI/data after logout -> login)
    location.reload();
    return;
...
async function loadMembers() {
  ...
  try {
    ...
    const members = (data.members || []).map(m => ({
      ...
      photoUrl: m.photoUrl || '',
      position: m.position || '',
      // allow member-list entries to optionally mark admin; default false
      isAdmin: toBool(m.isAdmin),
      isFromAdminsSheet: false,
    }));
...
async function loadAttendance(showRefreshMessage = false, showLoading = true) {
  ...
  const response = await fetchWithTimeout(`${API_URL}?action=get_today_attendance&date=${selectedDate}`, {}, 10000);
    ...
    // Normalize attendance records so frontend can rely on `inTime`/`outTime`
    // even when backend uses firstIn/firstOut/secondIn/secondOut fields.
    currentAttendance = (currentAttendance || []).map(r => {
      try {
        const firstIn = r.firstIn || r.inTime || '';
        const firstOut = r.firstOut || r.outTime || '';
        const secondIn = r.secondIn || '';
        const secondOut = r.secondOut || '';

        let inTime = '';
        let outTime = '';

        // Active IN is an IN with no corresponding OUT
        if (firstIn && firstIn !== '-' && (!firstOut || firstOut === '')) {
          inTime = firstIn;
          outTime = firstOut || '';
        } else if (secondIn && secondIn !== '-' && (!secondOut || secondOut === '')) {
          inTime = secondIn;
          outTime = secondOut || '';
        } else {
          // No active IN — fall back to most recent values for display
          inTime = firstIn || secondIn || r.inTime || '';
          outTime = firstOut || secondOut || r.outTime || '';
        }

        return { ...r, inTime, outTime };
      } catch (e) {
        return r;
      }
    });
...
function displayAttendance() {
  ...
  // Check if there are any checked-in members (has a Morning/Afternoon IN but missing its corresponding OUT)
  const checkedInMembers = currentAttendance.filter(record => {
    const firstIn = record.firstIn || record.inTime || '';
    const firstOut = record.firstOut || record.outTime || '';
    const secondIn = record.secondIn || '';
    const secondOut = record.secondOut || '';
    const firstActive = firstIn && firstIn !== '' && firstIn !== '-' && (!firstOut || firstOut === '');
    const secondActive = secondIn && secondIn !== '' && secondIn !== '-' && (!secondOut || secondOut === '');
    return (firstActive || secondActive) && record.hasWebAccess === true;
  });
  
  // "Sign Out All" feature removed from UI (admin mass-sign-out disabled)
  ...
  currentAttendance.forEach((record, index) => {
    ...
    setTimeout(() => {
      ...
      // Status (calculated below per-row using four-scan fields)
      let status = '';
      let statusClass = '';
    ...
    const nameBlock = `
      ...
      // Prepare first/second IN/OUT with fallbacks to legacy fields
      const firstIn = record.firstIn || record.inTime || '';
      const firstOut = record.firstOut || record.outTime || '';
      const secondIn = record.secondIn || '';
      const secondOut = record.secondOut || '';
      ...
      const formatCell = (val) => {
        try {
          if (!val || val === '-' ) return `<div class="time primary">-</div>`;
          if (val === '---') return `<div class="time primary">---</div>`;

          // If it's a numeric epoch milliseconds (sent by backend), convert to Date
          if (typeof val === 'number') {
            const d = new Date(val);
            const opts = { hour: 'numeric', minute: '2-digit', second: '2-digit', hour12: true };
            return `<div class="time primary">${d.toLocaleTimeString(undefined, opts)}</div>`;
          }

          // If it's a Date object
          if (val instanceof Date) {
            const opts = { hour: 'numeric', minute: '2-digit', hour12: true };
            return `<div class="time primary">${val.toLocaleTimeString(undefined, opts)}</div>`;
          }

          // If it's a string, try parsing as date/time
          const s = ('' + val).trim();
          const parsed = new Date(s);
          if (!isNaN(parsed.getTime())) {
            const opts = { hour: 'numeric', minute: '2-digit', hour12: true };
            return `<div class="time primary">${parsed.toLocaleTimeString(undefined, opts)}</div>`;
          }

          // Fallback: already a formatted time string
          return `<div class="time primary">${s}</div>`;
        } catch (e) {
          return `<div class="time primary">-</div>`;
        }
      };

      // Determine status for four-scan model
      // Show 'Complete' when all four timestamps exist,
      // 'Checked In' when there's an IN without its matching OUT,
      // 'Checked Out' whenever there is any OUT recorded (morning or afternoon),
      // otherwise leave blank/pending.
      let multiStatus = '';
      let multiClass = '';
      const hasFirstIn = firstIn && firstIn !== '' && firstIn !== '-';
      const hasFirstOut = firstOut && firstOut !== '' && firstOut !== '-';
      const hasSecondIn = secondIn && secondIn !== '' && secondIn !== '-';
      const hasSecondOut = secondOut && secondOut !== '' && secondOut !== '-';
      if (hasFirstIn && hasFirstOut && hasSecondIn && hasSecondOut) {
        multiStatus = 'Complete';
        multiClass = 'complete';
      } else if ((hasFirstIn && !hasFirstOut) || (hasSecondIn && !hasSecondOut)) {
        multiStatus = 'Checked In';
        multiClass = 'in';
      } else if (hasFirstOut || hasSecondOut) {
        multiStatus = 'Checked Out';
        multiClass = 'out';
      } else {
        multiStatus = '';
        multiClass = '';
      ...
      row.innerHTML = `
          ...
          <td data-label="Morning In">${formatCell(firstIn)}</td>
          <td data-label="Morning Out">${formatCell(firstOut)}</td>
          <td data-label="Afternoon In">${formatCell(secondIn)}</td>
          <td data-label="Afternoon Out">${formatCell(secondOut)}</td>
          <td data-label="Status"><span class="status-badge ${multiClass}">${multiStatus}</span></td>
        `;
...
// Highlight the row for a given UID briefly after an update
function highlightRowByUID(uid) {
  try {
    const k = (uid || '').toString().trim().toUpperCase();
    const idx = (currentAttendance || []).findIndex(r => (r.uid || '').toString().trim().toUpperCase() === k);
    if (idx < 0) return;
    ...
    tr.style.backgroundColor = '#FFF2CC'; // light highlight
    setTimeout(() => {
...
async function loadMyRecordsForMonth(yearMonth) {
  ...
  try {
    ...
    // Helper to format timestamps to short time (e.g. "12:36 PM") for the user records table
    const fmtTime = (val) => {
      try {
        ...
        if (val instanceof Date) {
          const opts = { hour: 'numeric', minute: '2-digit', hour12: true };
          return val.toLocaleTimeString(undefined, opts);
        }
        const s = ('' + val).trim();
        const parsed = new Date(s);
        if (!isNaN(parsed.getTime())) {
          const opts = { hour: 'numeric', minute: '2-digit', second: '2-digit', hour12: true };
          return parsed.toLocaleTimeString(undefined, opts);
        }
        return s;
      } catch (e) {
        return '—';
      ...
      <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
        <thead>
          <tr style="background: rgba(102, 126, 234, 0.1); border-bottom: 2px solid rgba(102, 126, 234, 0.2);">
            ...
            <th style="padding: 12px 8px; text-align: left; color: var(--text); font-weight: 600;">Morning In</th>
            <th style="padding: 12px 8px; text-align: left; color: var(--text); font-weight: 600;">Morning Out</th>
            <th style="padding: 12px 8px; text-align: left; color: var(--text); font-weight: 600;">Afternoon In</th>
            <th style="padding: 12px 8px; text-align: left; color: var(--text); font-weight: 600;">Afternoon Out</th>
        ...
        <tbody>
          ${userRecords.map((r, index) => {
            ...
            const secondIn = r.secondIn || '';
            const secondOut = r.secondOut || '';
            const hasFirst = firstIn && firstIn !== '' && firstIn !== '-' ;
            const hasFirstOut = firstOut && firstOut !== '' && firstOut !== '-';
            const hasSecond = secondIn && secondIn !== '' && secondIn !== '-';
            const hasSecondOut = secondOut && secondOut !== '' && secondOut !== '-';
            const status = (hasFirst && hasFirstOut && hasSecond && hasSecondOut) ? 'Complete' : ((hasFirst && !hasFirstOut) || (hasSecond && !hasSecondOut) ? 'Checked In' : (hasFirstOut || hasSecondOut ? 'Checked Out' : 'Pending'));
            const statusColor = (status === 'Complete') ? 'var(--success-color)' : (status === 'Checked In' ? 'var(--info-color)' : 'var(--warning-color)');
            ...
            return `
              <tr style="background: ${bgColor}; border-bottom: 1px solid rgba(102, 126, 234, 0.1);">
                ...
                <td style="padding: 12px 8px; color: var(--text);">${fmtTime(firstIn)}</td>
                <td style="padding: 12px 8px; color: var(--text);">${fmtTime(firstOut)}</td>
                <td style="padding: 12px 8px; color: var(--text);">${fmtTime(secondIn)}</td>
                <td style="padding: 12px 8px; color: var(--text);">${fmtTime(secondOut)}</td>
...
function updateStats() {
  ...
  const checkedIn = currentAttendance.filter(r => {
    const firstIn = r.firstIn || r.inTime || '';
    const firstOut = r.firstOut || r.outTime || '';
    const secondIn = r.secondIn || '';
    const secondOut = r.secondOut || '';
  ...
  const checkedOut = currentAttendance.filter(r => {
    const firstIn = r.firstIn || r.inTime || '';
    const firstOut = r.firstOut || r.outTime || '';
    const secondIn = r.secondIn || '';
    const secondOut = r.secondOut || '';
    ...
    // If user is currently checked-in (either morning or afternoon), they are NOT considered checked-out
    if (firstActive || secondActive) return false;

    // Otherwise, count them as checked-out if they have any valid OUT (Date or non-placeholder string)
    try { if (firstOut instanceof Date) return true; } catch (e) {}
    try { if (secondOut instanceof Date) return true; } catch (e) {}
    const fo = (firstOut || '').toString().trim();
    const so = (secondOut || '').toString().trim();
    const validFO = fo !== '' && fo !== '-' && fo !== '---';
    const validSO = so !== '' && so !== '-' && so !== '---';
    return validFO || validSO;
  }).length;
...
// Update member stats (exclude admins and head admin)
...
async function openRemoveMemberModal() {
  ...
  try {
    ...
    if (allMembers.length === 0) {
      ...
      const isSuperAdmin = currentUser && (currentUser.position || '').toLowerCase() === 'head admin';
      
      allMembers.forEach(member => {
        const memberCard = document.createElement('div');
        memberCard.className = 'member-card';
        const memberPos = member.position || '';
        const isTargetAdmin = member.isAdmin || (memberPos.toLowerCase() === 'admin') || (memberPos.toLowerCase() === 'head admin');
        const adminBadge = isTargetAdmin ? '<span class="admin-badge">ADMIN</span>' : '';
        const disabledAttr = (isTargetAdmin && !isSuperAdmin) ? 'disabled' : '';
        const disabledTitle = (isTargetAdmin && !isSuperAdmin) ? ' title="Only Head Admins can remove other admins"' : '';
        memberCard.innerHTML = `
          <div class="member-info">
            <div class="member-name">${member.name} ${adminBadge}</div>
            ${memberPos ? `<div class="member-position" style="font-size:12px; color: var(--muted);">${memberPos}</div>` : ''}
          </div>
...
async function removeMember(uid, name) {
  ...
  const isTargetAdmin = member && (member.isAdmin || memberPos === 'admin' || memberPos === 'head admin');
  
  // Check if current user is Head Admin - must have explicit "Head Admin" position or UID
  const currentUserPos = (currentUser.position || '').toLowerCase();
  const isSuperAdmin = currentUserPos === 'head admin' || currentUser.uid === 'FFDFA0DA';
  
  // Block removal of any admin by non-Head Admin
  if (isTargetAdmin) {
    if (!isSuperAdmin) {
      showNotification('⚠️ Only Head Admins can remove other admins: ' + name, 'error');
      return;
    }
    // Extra confirmation for Head Admin removing another admin
  ...
  try {
    ...
    if (data.status === 'success') {
      ...
      throw new Error('Only Head Admins can remove other admins');
    } else {
    ...
    <div class="modal-content" style="max-width: 640px; background: var(--card); color: var(--text);">
      ...
      <div class="modal-body" style="text-align:left; max-height: 65vh; overflow-y:auto;">
        <div class="form-group">
          ...
          <small style="color: var(--muted); font-size: 12px; display:block; margin-top:6px;">Admins can edit Members. Only Head Admin can edit Admins.</small>
        </div>
        <div class="form-group">
          <label>UID (Card Number)</label>
...
async function showEditMemberModal() {
  ...
  try {
    ...
    saveBtn.onclick = async () => {
      ...
      // Deny editing other admins (by sheet or by position) unless Head Admin
      const currentUserPos = (currentUser && currentUser.position ? currentUser.position : '').toString().trim().toLowerCase();
      const isSuperAdmin = (currentUser && currentUser.uid === 'FFDFA0DA') || currentUserPos === 'head admin';
      ...
      const isTargetAdmin = !!(targetEntry && (targetEntry.isAdmin === true || targetPos === 'admin' || targetPos === 'head admin'));
        ...
        msgEl.innerHTML = `
          <div style="background: var(--danger-bg, #f8d7da); border: 1px solid var(--danger-border, #dc3545); border-radius: 8px; padding: 12px; margin-top: 8px;">
            ...
            <p style="color: var(--danger-color, #721c24); font-size: 13px; margin: 0;">
              Only the Head Admin can modify another admin's information (including admins assigned by position).
            </p>
          </div>
        `;
        return;
      ...
      try {
        ...
        if (data.status === 'success') {
          ...
          setTimeout(() => {
            ...
            // Do NOT reopen modal after edit
          }, 800);
        } else {
...
function showEditPositionModal() {
  ...
  // Check if current user is head admin (by position or by UID)
  const currentUserPos = (currentUser.position || '').toLowerCase();
  const isSuperAdmin = currentUserPos === 'head admin' || currentUser.uid === 'FFDFA0DA';

  // Hide/show admin positions based on head admin status
  ...
  errorEl.textContent = '';
  if (warningEl) {
    warningEl.innerHTML = '<div style="color: var(--muted); font-size: 12px;">Note: Only <strong>Head Admin</strong> can assign Admin/Head Admin roles or change another admin\'s position.</div>';
  }
  ...
  // Add admins group if head admin
  ...
  function updateAdminSelectionWarning() {
    try {
      ...
      const isTargetAdmin = !!(targetEntry && (targetEntry.isAdmin === true || tPos === 'admin' || tPos === 'head admin'));

      // Any admin can edit normal members, only Head Admin can edit other admins
          ...
          warningEl.innerHTML = `
            <div style="background: var(--danger-bg, #f8d7da); border: 1px solid var(--danger-border, #dc3545); border-radius: 8px; padding: 12px; margin-top: 8px;">
              <p style="color: var(--danger-color, #721c24); font-size: 14px; font-weight: 600; margin: 0 0 6px 0;">
                ...
                <p style="color: var(--danger-color, #721c24); font-size: 13px; margin: 0;">
                Only the Head Admin can modify another admin's position (including admins assigned by position).
              </p>
            </div>`;
        }
  ...
  btnSave.onclick = async () => {
    ...
    if (position === 'custom') {
      ...
      // Block reserved words "Admin" and "Head Admin" in custom positions
      const positionLower = position.toLowerCase();
      if (positionLower.includes('admin') || positionLower.includes('head admin')) {
        errorEl.textContent = '⚠️ Cannot use "Admin" or "Head Admin" in custom positions. Use the dropdown to assign admin roles.';
        return;
      }
    }

    // Re-check head admin status for save operation
    const currentUserPos = (currentUser.position || '').toLowerCase();
    const isCurrentUserSuperAdmin = currentUserPos === 'head admin' || currentUser.uid === 'FFDFA0DA';

    // Check if trying to change another admin's position (only Head Admin can do this)
    ...
    const isTargetAdmin = !!(targetMember && (targetMember.isAdmin === true || targetPos === 'admin' || targetPos === 'head admin'));
    
    // Block changing admin positions unless current user is Head Admin
    ...
    // Check if trying to assign admin position without Head Admin privileges
    const isAdminPosition = position === 'Admin' || position === 'Head Admin';
    if (isAdminPosition && !isCurrentUserSuperAdmin) {
      errorEl.textContent = 'Only Head Admin can assign administrative positions';
      return;
    }
    ...
    try {
      ...
      if (data.status === 'success') {
        ...
        const promotedToAdmin = (position === 'Admin' || position === 'Head Admin');
        ...
        setTimeout(() => {
          ...
          errorEl.textContent = 'Only Head Admin (Kim Carlo Tolentino) can assign administrative positions';
        } else {
        ...
        return `
          <div class="member-card" style="display: flex; align-items: center; gap: 12px;">
            ...
            <div class="member-info" style="flex: 1;">
              ...
              ${position ? `<div class="member-position" style="font-size:12px; color: var(--muted);">${position}</div>` : ''}
            </div>
...
async function performRemoveMember(uid, name, modal) {
  ...
  if (isTargetAdmin) {
    if (!isSuperAdmin) {
      showNotification('⚠️ Only Head Admins can remove other admins', 'error');
      return;
    }
    // Extra confirmation for Head Admin removing another admin
    if (!confirm(`⚠️ WARNING: You are about to remove an ADMIN.\n\nUser: ${name}\nUID: ${uid}\n\nThis action cannot be undone. Continue?`)) {
  ...
  try {
    ...
    if (data.status === 'success') {
      ...
      modal.style.display = 'none';
      loadMembers();
      // Do NOT reopen modal after removal
    } else if (data.status === 'forbidden') {
      showNotification('Only Head Admins can remove other admins', 'error');
    } else {
      throw new Error(data.message || 'Failed to remove member');
    }
  } catch (error) {
...
async function showResetTodayOutModal() {
  ...
  try {
    ...
    // Filter members who have OUT time today (original behavior)
    ...
    // Display members with OUT time (original behavior)
...
async function performResetOutTime(uid, name) {
  ...
  try {
    ...
    } else if (result === 'afternoon_out_cleared') {
      showNotification(`Afternoon OUT time reset for ${name}!`, 'success');
      successDiv.textContent = `✅ Afternoon OUT time reset for ${name}`;
      await loadAttendance();
      updateWebAttendanceStatus();
      setTimeout(() => { showResetTodayOutModal(); }, 1200);
    } else {
...
// Sign Out All feature removed — admin mass-sign-out disabled
...
// Sign-in cooldown helpers (1 minute default)
let signInCooldownInterval = null;
function setSignInCooldown(uid, ms = 60000) {
  try { localStorage.setItem(`signInCooldown_${uid}`, String(Date.now() + ms)); } catch {}
}
function getSignInCooldownRemaining(uid) {
  try {
    const exp = parseInt(localStorage.getItem(`signInCooldown_${uid}`) || '0', 10);
    return Math.max(0, exp - Date.now());
  } catch { return 0; }
}
function clearSignInCooldown(uid) {
  try { localStorage.removeItem(`signInCooldown_${uid}`); } catch {}
}

function updateWebAttendanceStatus() {
  if (!currentUser) return;
  // Hide personal Sign In/Out buttons for admins (detect multiple truthy formats)
  const uid = (currentUser.uid || '').toString().trim().toUpperCase();
  const isTruthyAdmin = (v) => {
  ...
  // Determine if current user is listed specifically in the Admins sheet
  const isInAdminSheet = !!(allMembers || []).find(m => {
    try {
      const k = (m.uid || '').toString().trim().toUpperCase();
      return k === uid && m.isFromAdminsSheet === true;
    } catch { return false; }
  });

  // Also respect a direct session flag (legacy); but only treat it as admin-sheet
  // if the session user was actually an admins-sheet entry. This prevents member-list
  // admins from being hidden.
  const sessionIsAdminFromSheet = !!(currentUser && isTruthyAdmin(currentUser.isAdmin) &&
    !!(allMembers || []).find(m => (m.uid || '').toString().trim().toUpperCase() === uid && m.isFromAdminsSheet === true)
  );

  if (isInAdminSheet || sessionIsAdminFromSheet) {
  ...
  try {
    ...
    if (myRecord) {
      // If Afternoon Out filled, hide both buttons
      const hasAfternoonOut = (myRecord.secondOut && myRecord.secondOut !== '' && myRecord.secondOut !== '-');
      if (hasAfternoonOut) {
        btnSignIn.style.display = 'none';
        btnSignOut.style.display = 'none';
        if (signOutCooldownInterval) { clearInterval(signOutCooldownInterval); signOutCooldownInterval = null; }
        if (signInCooldownInterval) { clearInterval(signInCooldownInterval); signInCooldownInterval = null; }
        return;
      }
      ...
      if (myRecord.inTime && myRecord.inTime !== '-' && (!myRecord.outTime || myRecord.outTime === '-')) {
        ...
        // User completed a check-in/out pair. Allow Sign In again after a short cooldown (1 min).
        btnSignOut.style.display = 'none';
        const remainingSignIn = getSignInCooldownRemaining(currentUser.uid);
        if (remainingSignIn > 0) {
          btnSignIn.style.display = 'inline-block';
          btnSignIn.disabled = true;
          // Start/update countdown overlay on sign-in button
          const updateSignInLabel = () => {
            const r = getSignInCooldownRemaining(currentUser.uid);
            if (r <= 0) {
              if (signInCooldownInterval) { clearInterval(signInCooldownInterval); signInCooldownInterval = null; }
            ...
            const mm = String(Math.floor(secs / 60)).padStart(1, '0');
            const ss = String(secs % 60).padStart(2, '0');
            btnSignIn.textContent = `Sign In (${mm}:${ss})`;
          };
          updateSignInLabel();
          if (!signInCooldownInterval) signInCooldownInterval = setInterval(updateSignInLabel, 1000);
        } else {
          btnSignIn.style.display = 'inline-block';
          btnSignIn.disabled = false;
          if (signInCooldownInterval) { clearInterval(signInCooldownInterval); signInCooldownInterval = null; }
...
async function performWebSignIn() {
  ...
  try {
    ...
    if (result === 'signed_in' || result.includes('success') || result === 'signed_in_afternoon') {
      if (result === 'signed_in_afternoon') {
        showNotification('Successfully signed in (Afternoon)!', 'success');
      } else {
      ...
      await loadAttendance(false);
      // Highlight the updated row if it was an afternoon sign-in
      if (result === 'signed_in_afternoon') {
        highlightRowByUID(currentUser.uid);
      }
      updateWebAttendanceStatus();
    } else {
      throw new Error(result);
...
async function performWebSignOut() {
  ...
  try {
    ...
    if (result === 'signed_out' || result.includes('success')) {
      ...
      // After signing out, allow Sign In again but enforce a short 1-minute cooldown
      setSignInCooldown(currentUser.uid, 60000);
      await loadAttendance(false);
      updateWebAttendanceStatus();
    } else {
      throw new Error(result);
...
async function showWebAccessModal() {
  ...
  try {
    ...
    // Use allMembers array for consistent position data
    const membersHTML = (allMembers || []).filter(m => !m.isAdmin).map(member => {
      ...
      return `
        <div class="member-card" style="display: flex; justify-content: space-between; align-items: center; padding: 15px; margin-bottom: 10px; border: 1px solid var(--border); border-radius: 8px; background: var(--card);">
          <div style="display: flex; align-items: center; gap: 12px; flex: 1;">
            ...
            <div class="member-info" style="display: flex; flex-direction: column; gap: 2px;">
              <div class="member-name" style="font-weight: 600; color: var(--text);">${name}</div>
              <span class="member-position" style="font-size: 12px; color: var(--muted);">${position}</span>
...
async function toggleWebAccess(uid, name, currentAccess) {
  ...
  try {
    ...
    if (result === 'success' || result.includes('updated')) {
      ...
      // Update only the button and status for the affected member
      const memberCard = document.querySelector(`.member-card button[onclick*="${uid}"]`).closest('.member-card');
      if (memberCard) {
        const hasWebAccess = !currentAccess;
        const statusDiv = memberCard.querySelector('.member-info > div[style*="margin-top: 5px"] span');
        if (statusDiv) {
          statusDiv.textContent = hasWebAccess ? '✅ Web Access Granted' : '❌ No Web Access';
          statusDiv.style.color = hasWebAccess ? 'var(--success-color)' : 'var(--danger-color)';
        }
        const btn = memberCard.querySelector('button.btn-remove');
        if (btn) {
          btn.textContent = hasWebAccess ? 'Revoke Access' : 'Grant Access';
          btn.setAttribute('onclick', `toggleWebAccess('${uid}', '${name.replace(/'/g, "&#39;")}', ${hasWebAccess})`);
        }
      }
    } else {
      throw new Error(result);
    ...
    if (allMembers && allMembers.length > 0) {
          ...
          <div style="display: flex; justify-content: space-between; align-items: center; gap: 12px;">
            <div style="display: flex; align-items: center; gap: 12px; flex: 1;">
              ...
              <div style="display: flex; flex-direction: column; gap: 2px;">
                ...
                <span style="font-size: 12px; color: var(--muted);">${position}</span>
              </div>
            </div>
      ...
      contentDiv.innerHTML = `
        <div style="padding-bottom: 10px; border-bottom: 1px solid rgba(102, 126, 234, 0.2); margin-bottom: 10px;">
          <p style="color: var(--muted); font-size: 13px;"><strong>Total Members:</strong> ${allMembers.length}</p>
        </div>
        ${membersHTML}
      `;
    } else {
