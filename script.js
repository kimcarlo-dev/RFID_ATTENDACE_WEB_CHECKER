// API Configuration
const API_URL = 'https://script.google.com/macros/s/AKfycbxxDhYoB3KHeoS6Ydfob8S6WF8rJl0ZZJw3Ik2DxRBFKoI9kGztkUDsmwqFZwbugd4zFQ/exec';
    ...
    try {
      ...
      try {
        ...
        if (person) {
          ...
          currentUser.isAdmin = !!person.isAdmin;
          currentUser.hasWebAccess = person.isAdmin ? false : person.webAccess === true;
...
async function handleLogin(e) {
  ...
  const passwordResponse = await fetch(`${API_URL}?action=verify_password&uid=${encodeURIComponent(uid)}&password=${encodeURIComponent(password)}`);
    ...
    currentUser = {
      ...
      hasWebAccess: person.isAdmin ? false : person.webAccess === true,
...
async function showEditMemberModal() {
  ...
  try {
    ...
    saveBtn.onclick = async () => {
      ...
      // Check if user is editing their own UID
...
function showEditPositionModal() {
  ...
  const isSuperAdmin = currentUser.uid === 'FFDFA0DA';
  ...
  btnSave.onclick = async () => {
    ...
    if (isAdminPosition && !isSuperAdmin) {
...
async function performRemoveMember(uid, name, modal) {
  // Block removal of admins - check from allMembers data
