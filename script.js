// API Configuration
const API_URL = 'https://script.google.com/macros/s/AKfycbzxEHw1Vp8-BiQwytD0DOS2I9UFwORWwV904ONMQCq5dQW-W4F5RfCFUiVi4S7V2-T5gQ/exec';
...
async function checkSession() {
  ...
  console.log('Checking session:', session); // Debug log
  if (session) {
    try {
      currentUser = JSON.parse(session);
      console.log('Session restored:', currentUser); // Debug log
      
      // Refresh user data from server to get latest webAccess status
      try {
        const response = await fetch(`${API_URL}?action=list_members`);
        const data = await response.json();
        const member = (data.members || []).find(m => 
          (m.uid || '').toString().trim().toUpperCase() === currentUser.uid
        );
        
        if (member) {
          currentUser.hasWebAccess = member.webAccess === true;
    ...
    console.log('No session found, showing login'); // Debug log
    showLogin();
  }
}
...
async function handleLogin(e) {
  ...
  const passwordResponse = await fetch(`${API_URL}?action=verify_password&uid=${encodeURIComponent(uid)}&password=${encodeURIComponent(password)}`);
    ...
    currentUser = {
      ...
      hasWebAccess: member.webAccess === true
    };
...
function showDashboard() {
  ...
  // Show/hide web attendance buttons for non-admin members with web access
  const btnWebSignIn = document.getElementById('btnWebSignIn');
  const btnWebSignOut = document.getElementById('btnWebSignOut');
  
  if (!currentUser.isAdmin && currentUser.hasWebAccess) {
    // Initially hide both, updateWebAttendanceStatus will show the appropriate one
...
async function loadMembers() {
  try {
    const response = await fetchWithTimeout(`${API_URL}?action=list_members`, {}, 10000);
    const data = await response.json();
    allMembers = (data.members || []).map(m => ({
      ...m,
      isAdmin: ADMIN_UIDS.includes((m.uid || '').toString().trim().toUpperCase()),
      hasWebAccess: m.webAccess === true
    }));
    updateMemberStats();

    // After members are loaded, refresh the header photo so it can
    // resolve the current user's image from the latest members list.
    // This fixes the case where the Attendance table shows photos
    // correctly (because it uses records), but the header keeps the
    // placeholder on first render before members are loaded.
    if (currentUser) {
  ...
  const response = await fetchWithTimeout(`${API_URL}?action=get_today_attendance&date=${selectedDate}`, {}, 10000);
    ...
    if (Array.isArray(currentAttendance) && allMembers && allMembers.length) {
      ...
      allMembers.forEach(m => {
        ...
        if (k) accessMap.set(k, m.webAccess === true);
      });
...
function displayAttendance() {
  ...
  // Show "Sign Out All" button only if there are checked-in members and user is admin
  if (btnSignOutAll) {
    if (currentUser && currentUser.isAdmin && checkedInMembers.length > 0) {
...
async function performWebSignIn() {
  ...
  try {
    ...
    if (result === 'signed_in' || result.includes('success')) {
      ...
      await updateWebAttendanceStatus();
      await loadAttendance(false);
    } else {
      throw new Error(result);
    }
  ...
  try {
    ...
    if (data.status === 'success' && data.members.length > 0) {
      ...
      const membersHTML = data.members.map((m, i) => {
        ...
        const uid = (m.uid||'')+''
        const initial = name ? name[0].toUpperCase() : '?';
        const isAdmin = ADMIN_UIDS.includes((uid).toUpperCase());
